...
//需要修改的地方
#define  sw_write_byte(dat)  bluetooth_ch9141_send_byte(dat)//串口发送字节
#define  sw_write_buffer(dat,len)  bluetooth_ch9141_send_buffer(dat,len)//无线串口发送数组或串口发送数组
...
void sendimgAndLine_type(uint8 color,uint8 *buff, uint32 len,uint8 size_lon)
{
    ...
    Line_Buffer[Line_lon]=(255);Line_lon++;
    Line_Buffer[Line_lon]=(255);Line_lon++;
    for(int i=0;i<len*size_lon;i+=size_lon){ Line_Buffer[Line_lon]=*(buff+i);Line_lon++;}
}
...
//ccd数据发送到上位机上，需要点击上位机上方ccd同传
void put_ccd_ch1(uint16* dat)
{
  uint8 ccd_head[]={0x22,0x6B,0x09,0x6c,0x10,0x01};//帧头
  sw_write_buffer(ccd_head,6);
  sw_write_buffer((uint8*)dat,128*2);//数据 uint16->uint8 长度x2
}
...
//str要发送的字符串  lon数据长度
//例:put_string("hellow",6);
//   char *str="hellow";
//put_string(str,sizeof(str));
void put_string(char*str,int lon)
{
    uint8 head_buff[5]={197,1,200,233,23};
    uint8 end_buff[5]={(uint8)(lon & 0xFF), (uint8)((lon & 0xFF00) >> 8),197,0,254};
    sw_write_buffer(head_buff,5);
    sw_write_buffer(str,lon);
    sw_write_buffer(end_buff,5);
}
